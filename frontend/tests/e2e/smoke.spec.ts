...
test('debug logs and Vue monitor use temporary diagnostics data', async ({ page }) => {
  const external:string[]=[];page.on('request',r=>{if(!['127.0.0.1','localhost'].includes(new URL(r.url()).hostname))external.push(r.url())});await page.addInitScript(()=>{localStorage.setItem('llm-relay-desk.locale','en-US');class FakeWebSocket{static instances:FakeWebSocket[]=[];onopen:(()=>void)|null=null;onmessage:((event:{data:string})=>void)|null=null;onerror:(()=>void)|null=null;onclose:(()=>void)|null=null;constructor(){FakeWebSocket.instances.push(this);setTimeout(()=>{this.onopen?.();this.onmessage?.({data:JSON.stringify({type:'snapshot',requests:[{request_id:'e2e',api:'openai',route:'/v1/chat/completions',model:'mock/model',status:'streaming',content:'Final answer',reasoning:'Reasoning text',started_at:'2026-01-01T00:00:00Z'}]})})},10)}close(){} }Object.assign(globalThis,{WebSocket:FakeWebSocket,__monitorSockets:FakeWebSocket.instances})})
  const list={enabled:true,directory:'C:/temporary',retention_files:10,file_count:1,logs:[{id:'temp.json',timestamp:'2026-01-01T00:00:00Z',request_id:'req-temp',status_code:200,outcome:'completed',size_bytes:100}]};await page.route('**/admin/debug-logs',r=>r.fulfill({json:r.request().method()==='DELETE'?{ok:true,removed:1,status:{...list,file_count:0,logs:[]}}:list}));await page.route('**/admin/debug-logs/temp.json',r=>r.fulfill({json:{timestamp:'2026-01-01T00:00:00Z',request_id:'req-temp',client_request:{body:{message:'hello'}},upstream_request:{body:{model:'mock/model'}},upstream_response:{body:{content:'answer'}}}}));await page.route('**/admin/monitor/history',r=>r.fulfill({json:{ok:true}}))
  await page.goto('/ui/debug-logs');await expect(page.getByRole('heading',{name:'Debug Logs'})).toBeVisible();await page.getByText('req-temp').click();await expect(page.getByText(/mock\/model/)).toBeVisible();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'Delete log'}).click();await page.goto('/ui/monitor');await expect(page.getByRole('heading',{name:'Realtime Monitor'})).toBeVisible();await expect(page.getByText('Reasoning text')).toBeVisible();await expect(page.getByText('Final answer')).toBeVisible();await page.getByRole('button',{name:'Reconnect'}).click();expect(await page.evaluate(()=>((globalThis as unknown as{__monitorSockets:unknown[]}).__monitorSockets.length))).toBeGreaterThan(1);expect(external).toEqual([])
})
